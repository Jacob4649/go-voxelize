...
// Small tweaks on github.com/jblindsay/lidario
...
func NewLasFile(fileName, fileMode string) (*LasFile, error) {
	...
	if las.fileMode == "r" || las.fileMode == "rh" {
		...
		var err error
		if las.RawFile, err = os.Create(las.fileName); err != nil {
			return &las, err
		}
...
func (las *LasFile) read() error {
	var err error
	if las.RawFile, err = os.Open(las.fileName); err != nil {
		return err
	}
...
func (las *LasFile) readHeader() error {
	...
	defer las.Unlock()
	b := make([]byte, 243)
	if _, err := las.RawFile.ReadAt(b[0:243], 0); err != nil && err != io.EOF {
		return err
	}
...
func (las *LasFile) readVLRs() error {
	...
	if _, err := las.RawFile.ReadAt(b, int64(las.Header.HeaderSize)); err != nil && err != io.EOF {
		return err
	}
...
func (las *LasFile) readPoints() error {
	...
	if _, err := las.RawFile.ReadAt(b, int64(las.Header.OffsetToPoints)); err != nil && err != io.EOF {
		return err
	}
